...
module.exports = function(p) {
    ...
    if (/\[\?\] Select station:/.test(p.data)) {
        ...
        if (!p.options.loggedIn) p.write(p.options.station);
        p.emit('selectStation');
        p.setState({
            running: true,
            loggedIn: true,
            stations: p.data.replace(/ [q ][Q ] /g, '').match(/\d+\) .+[\S]+/g)
        });
        if (p.options.debug) console.log('Select station');
    }
    ...
    // [?] Password:
        ...
        if (p.options.debug) console.log('Enter password');
    }
    // [?] Email:
        ...
        if (p.options.debug) console.log('Enter email');
    }
    // |>  "Die Young" by "Kesha" on "Warrior (Deluxe Version)" @ Today's Hits
        ...
        if (p.options.debug) console.log('Song change');
    }
    // |>  Station "QuickMix" (1057370371552570017)
        ...
        if (p.options.debug) console.log('Station Changed');
    }
    /*
    ...
    //(i) No history yet.
        ...
        if (p.options.debug) console.log('History');
    }
    // #   -04:24/04:31
        ...
        if (p.options.debug) console.log('Time');
    }
    // (i) Login...
